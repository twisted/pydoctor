...
class ZopeInterfaceModuleVisitor(astbuilder.ModuleVistor):
    ...
    schema_like_patterns = [("zope\.interface\.Attribute", extractAttributeDescription)]
    ...
    def visitAssign(self, node):
        ...
        if (
            len(node.nodes) != 1
            or not isinstance(node.nodes[0], ast.AssName)
            or not isinstance(node.expr, ast.CallFunc)
        ):
            return sup()
        ...
        def pushAttribute(docstring, kind):
            ...
            if attr.parentMod.sourceHref:
                attr.sourceHref = (
                    attr.parentMod.sourceHref + "#L" + str(attr.linenumber)
                )
        ...
        def handleSchemaField(kind):
            ...
            descriptions = [
                ...
                if isinstance(arg, ast.Keyword) and arg.name == "description"
            ]
            docstring = None
    ...
    def visitCallFunc_zope_interface_implements(self, funcName, node):
        ...
        addInterfaceInfoToClass(
            self.builder.current, node.args, funcName == "zope.interface.implementsOnly"
        )

    visitCallFunc_zope_interface_implementsOnly = (
        visitCallFunc_zope_interface_implements
    )

    def visitCallFunc_zope_interface_classImplements(self, funcName, node):
        clsname = self.builder.current.expandName(ast_pp.pp(node.args[0]))
        if clsname not in self.system.allobjects:
            self.builder.system.msg(
                "parsing", "classImplements on unknown class %r" % clsname
            )
            return
        cls = self.system.allobjects[clsname]
        addInterfaceInfoToClass(
            cls, node.args[1:], funcName == "zope.interface.classImplementsOnly"
        )

    visitCallFunc_zope_interface_classImplementsOnly = (
        visitCallFunc_zope_interface_classImplements
    )
    ...
    def visitClass(self, node):
        ...
        if len([b for b in cls.bases if namesInterface(self.system, b)]) > 0:
