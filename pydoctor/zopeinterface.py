...
class ZopeInterfaceModuleVisitor(astbuilder.ModuleVistor):
    ...
    def visitAssign(self, node):
        ...
        def handleSchemaField(kind):
            #print node.expr
            descriptions = [
                arg for arg in node.value.keywords
                if isinstance(arg, ast.keyword) and arg.arg == 'description'
            ]
            docstring = None
            if len(descriptions) > 1:
                self.builder.system.msg('parsing', 'xxx')
            if len(descriptions) == 1:
                docstring = extractStringLiteral(descriptions[0].value)
    ...
    def visitCall(self, node):
        ...
        meth = getattr(self, "visitCall_" + base.replace('.', '_'), None)
        if meth is not None:
            meth(base, node)

    def visitCall_zope_interface_moduleProvides(self, funcName, node):
    ...
    def visitCall_zope_interface_implements(self, funcName, node):
    ...
    visitCall_zope_interface_implementsOnly = visitCall_zope_interface_implements

    def visitCall_zope_interface_classImplements(self, funcName, node):
    ...
    visitCall_zope_interface_classImplementsOnly = visitCall_zope_interface_classImplements

    def visitClassDef(self, node):
        super(ZopeInterfaceModuleVisitor, self).visitClassDef(node)
