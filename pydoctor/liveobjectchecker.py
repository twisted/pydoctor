...
def loadModulesForSystem(system):
    ...
    try:
        ...
        modlist = [
            system.allobjects[m]
            for m in astbuilder.toposort(
                [m.fullName() for m in system.objectsOfType(model.Module)],
                system.importgraph,
            )
        ]
        errcount = 0
...
def typeChecker(builder, name, OBJ):
    ...
    while not isinstance(mod, model.Module):
        mod = mod.parent
    if (
        getattr(OBJ, "__module__", None) != mod.fullName()
        or getattr(OBJ, "__name__", None) != name
    ):
        return
