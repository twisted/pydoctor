...
def get_parser() -> ArgumentParser:
    ...
    parser.add_argument(
        ...
        action='store_true',
        dest='makehtml',
        default=Options.MAKE_HTML_DEFAULT,
        help=(
            "Produce html output."
    ...
    parser.add_argument(
        ...
        action='append',
        dest='privacy',
        metavar='<PRIVACY>:<PATTERN>',
        default=[],
        help=(
            "Set the privacy of specific objects when default rules doesn't fit the use case. "
    ...
    parser.add_argument(
        ...
        dest='htmlsourcetemplate',
        help=(
            "A format string used to generate the source link of documented objects. "
        ...
        metavar='SOURCETEMPLATE',
        default=Options.HTML_SOURCE_TEMPLATE_DEFAULT,
    )
    parser.add_argument(
        '--html-base-url',
        dest='htmlbaseurl',
        help=(
            "A base URL used to include a canonical link in every html page. "
            "This help search engine to link to the preferred version of "
            "a web page to prevent duplicated or oudated content. "
        ),
        default=None,
        metavar='BASEURL',
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        dest='sidebarexpanddepth',
        help=(
            "How many nested modules and classes should be expandable, "
    ...
    parser.add_argument(
        '--cls-member-order',
        dest='cls_member_order',
        default="alphabetical",
        choices=["alphabetical", "source"],
        help=("Presentation order of class members. (default: alphabetical)"),
    )
    parser.add_argument(
        '--mod-member-order',
        dest='mod_member_order',
        default="alphabetical",
        choices=["alphabetical", "source"],
    ...
    parser.add_argument(
        ...
        dest='use_hardlinks',
        help=(
            "Always copy files instead of creating a symlink (hardlinks will be automatically used if the symlink process failed)."
        ),
    )
...
class Options:
    ...
    htmloutput: str = (
        attr.ib()
    )  # TODO: make this a Path object once https://github.com/twisted/pydoctor/pull/389/files is merged
    ...
    def __attrs_post_init__(self) -> None:
        ...
        if self.sidebarexpanddepth < 1:
            error(
                "Invalid --sidebar-expand-depth value."
                + 'The value of --sidebar-expand-depth option should be greater or equal to 1, '
                'to suppress sidebar generation all together: use --no-sidebar'
            )
        if self.sidebartocdepth < 0:
            error(
                "Invalid --sidebar-toc-depth value"
                + 'The value of --sidebar-toc-depth option should be greater or equal to 0, '
                'to suppress sidebar generation all together: use --no-sidebar'
            )
