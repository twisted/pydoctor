...
class SideBarSection(Element):
    ...
    def description(self) -> str:
        ...
        """
        return (
            f"This {epydoc2stan.format_kind(self.documented_ob.kind).lower() if self.documented_ob.kind else 'object'}"
            if self._represents_documented_ob
            else (
...
class ObjContent(Element):
    ...
    def has_contents(self) -> bool:
        return bool(
            self.classList
            or self.functionList
            or self.variableList
            or self.subModuleList
            or self.inheritedFunctionList
            or self.inheritedVariableList
        )
...
class ContentList(TemplateElement):
    ...
    def __init__(
        self,
        ob: Documentable,
        children: Iterator[Documentable],
        documented_ob: Documentable,
        expand: bool,
        nested_content_loader: ITemplateLoader,
        template_lookup: TemplateLookup,
