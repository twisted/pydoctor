...
def getparser():
    ...
    parser.add_option(
        ...
        dest="inputpickle",
        help=(
            "Load the system from this pickle file (default: "
    ...
    parser.add_option(
        ...
        dest="outputpickle",
        help=(
            "Save the system to this pickle file (default: "
    ...
    parser.add_option(
        ...
        help=(
            "Absolute path to the base directory of the "
    ...
    parser.add_option(
        "--make-intersphinx",
        action="store_true",
        dest="makeintersphinx",
        default=False,
        help=("Produce (only) the objects.inv intersphinx file."),
    )
    parser.add_option(
        "--add-package",
        action="append",
        ...
        default=[],
        help=(
            "Add a package to the system.  Can be repeated "
    ...
    parser.add_option(
        ...
        dest="livecheck",
        default=False,
        help=("Import and examine the modules too.  XXX not working " "right now"),
    )
    parser.add_option(
        "-v",
        "--verbose",
    ...
    parser.add_option(
        ...
        action="store_true",
        default=False,
        help=("Clear the Intersphinx cache " "specified by --intersphinx-cache-path."),
    )
    parser.add_option(
    ...
    parser.add_option(
        "--hide-private",
        dest="hide_private",
        default=HIDE_PRIVATE_HELP_DEFAULT,
        action="store_true",
        help=HIDE_PRIVATE_HELP,
    )
...
def readConfigFile(options):
    ...
    for i, line in enumerate(open(options.configfile, "rU")):
        ...
        if not line or line.startswith("#"):
            continue
        if ":" not in line:
            error("don't understand line %d of %s", i + 1, options.configfile)
        k, v = line.split(":", 1)
        k = k.strip()
        v = os.path.expanduser(v.strip())

        if not hasattr(options, k):
            error("invalid option %r on line %d of %s", k, i + 1, options.configfile)
...
def main(args):
    ...
    try:
        ...
        if options.inputpickle:
            ...
            if options.systemclass:
                if type(system) is not systemclass:
                    cls = type(system)
                    msg = (
                        "loaded pickle has class %s.%s, differing "
        ...
        if args:
            ...
            if options.prependedpackage:
                for m in options.prependedpackage.split("."):
                    prependedpackage = system.Package(system, m, None, prependedpackage)
                    system.addObject(prependedpackage)
                    initmodule = system.Module(
                        system, "__init__", None, prependedpackage
                    )
        ...
        if not system.packages:
            error(
                "The system does not contain any code, did you "
        ...
        if system.options.projectname is None:
            ...
            system.msg(
                "warning", "WARNING: guessing " + name + " for project name", thresh=-1
            )
        ...
        if options.makehtml:
            ...
            if system.epytextproblems:
                ...
                def p(msg):
                    system.msg(
                        ("epytext", "epytext-summary"), msg, thresh=-1, topthresh=1
                    )

                p(
                    "these %s objects' docstrings are not proper epytext:"
        ...
        if options.makeintersphinx:
            ...
            sphinx_inventory = SphinxInventory(
                logger=system.msg, project_name=system.projectname
            )
            if not os.path.exists(options.htmloutput):
                os.makedirs(options.htmloutput)
            sphinx_inventory.generate(subjects=subjects, basepath=options.htmloutput)
    except:
        if options.pdb:
            import pdb
