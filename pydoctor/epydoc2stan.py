...
def get_parser(formatname):
    try:
        mod = __import__(
            "epydoc.markup." + formatname, globals(), locals(), ["parse_docstring"]
        )
...
class _EpydocLinker(object):
    ...
    def look_for_name(self, name, candidates):
        ...
        if len(potential_targets) == 1:
            ...
            self.obj.system.msg(
                "translate_identifier_xref",
                "%s:%s ambiguous ref to %s, could be %s"
                % (
                    self.obj.fullName(),
                    self.obj.linenumber,
                    name,
...
class Field(object):
    ...
    def __repr__(self):
        ...
        if len(r) > 25:
            r = r[:20] + "..." + r[-2:]
        return "<%s %r %r %s>" % (self.__class__.__name__, self.tag, self.arg, r)
...
def reportErrors(obj, errs):
    ...
    if errs and obj.fullName() not in obj.system.epytextproblems:
        ...
        obj.system.msg("epydoc2stan", "epytext error in %s" % (obj,), thresh=1)
...
def doc2stan(obj, summary=False, docstring=None):
    ...
    if getattr(obj, "parsed_docstring", None) is not None:
        ...
        if getattr(obj, "parsed_type", None) is not None:
            r = [
                r,
                " (type: ",
                html2stan(obj.parsed_type.to_html(_EpydocLinker(obj))),
                ")",
            ]
        return r, []
    origobj = obj
    ...
    if summary:
        ...
        lines = itertools.dropwhile(lambda line: not line.strip(), doc.split("\n"))
