...
from pydoctor.napoleon.docstring import (
    GoogleDocstring as _GoogleDocstring,
...
SphinxGoogleDocstring = partialclass(
    sphinx_napoleon.docstring.GoogleDocstring,
    config=sphinx_napoleon_config,
    what='function',
)
SphinxNumpyDocstring = partialclass(
    sphinx_napoleon.docstring.NumpyDocstring,
    config=sphinx_napoleon_config,
    what='function',
)
...
class BaseDocstringTest(TestCase):
    ...
    def assertAlmostEqualSphinxDocstring(
        self,
        expected: str,
        docstring: str,
        type_: Type[Union[SphinxGoogleDocstring, SphinxNumpyDocstring]],
        ...
        expected_sphinx_output = re.sub(
            r"(`|\\\s|\\|:mod:|:func:|:class:|:obj:|:py:mod:|:py:func:|:py:class:|:py:obj:)",
            "",
            expected,
        )
        ...
        sphinx_docstring_output = re.sub(
            ...
            "",
            str(type_(docstring))
            .replace(":kwtype", ":type")  # type: ignore[misc]
            .replace(":vartype", ":type")
            .replace(" -- ", " - ")
            .replace(':rtype:', ':returntype:')
            .rstrip(),
        )
...
class TypeDocstringTest(BaseDocstringTest):
    ...
    def test_is_google_typed_arg(self):
        ...
        self.assertFalse(
            is_google_typed_arg("List of string or any kind fo sequences of strings")
        )
        ...
        self.assertTrue(
            is_google_typed_arg("list of int or float or None, default: None")
        )
        self.assertTrue(
            is_google_typed_arg(
                "`complicated string` or `strIO <twisted.python.compat.NativeStringIO>`"
        ...
        self.assertFalse(
            is_google_typed_arg(
                "foo (List of string or any kind fo sequences of strings)"
            )
        )
        ...
        self.assertTrue(
            is_google_typed_arg("foo (list of int or float or None, default: None)")
        )
        self.assertTrue(
            is_google_typed_arg(
                "foo (`complicated string` or `strIO <twisted.python.compat.NativeStringIO>`)"
            )
        )

        self.assertTrue(
            is_google_typed_arg(
                "Random words are not a type spec (List[str] or list(bytes), optional)"
            )
        )
        self.assertTrue(
            is_google_typed_arg(
                "Random words are not a type spec (list of int or float or None, default: None)"
            )
        )
        self.assertTrue(
            is_google_typed_arg(
                "Random words are not a type spec (`complicated string` or `strIO <twisted.python.compat.NativeStringIO>`, optional)"
            )
        )
    ...
    def test_convert_numpy_type_spec(self):
        ...
        specs = (
            ...
            "int or float or None, default=None",
            "int or float or None, default = None",  # corner case
            "int or float or None, default None",
            "int, default None",
            '{"F", "C", "N"}',
            "{'F', 'C', 'N'}, default: 'N'",
...
""",
    ...
    def test_docstrings(self):
        for docstring, expected in self.docstrings:
            ...
            self.assertEqual(expected.rstrip(), actual)
            if (
                not 'Yield' in docstring and not 'Todo' in docstring
            ):  # The yield and todo sections are very different from sphinx's.
                self.assertAlmostEqualSphinxDocstring(
                    expected, docstring, type_=SphinxGoogleDocstring
                )
...
"""
        ...
        self.assertAlmostEqualSphinxDocstring(
            expected2, docstring2, type_=SphinxGoogleDocstring
        )
...
"""
        ...
        warning = doc.warnings.pop()
        self.assertIn(
            "invalid type: 'docformat (Can be \"numpy\"or \"google\")'", warning[0]
        )
        self.assertEqual(5, warning[1])
    ...
    def test_docstrings(self):
        ...
        for docstring, expected in self.docstrings:
            ...
            if (
                ...
                self.assertAlmostEqualSphinxDocstring(
                    expected, dedent(docstring), type_=SphinxNumpyDocstring
                )
...
"""
        self.assertEqual(expected.rstrip(), actual)
        self.assertAlmostEqualSphinxDocstring(
            expected, docstring, type_=SphinxNumpyDocstring
        )
...
"""
        ...
        self.assertAlmostEqualSphinxDocstring(
            expected, docstring2, type_=SphinxNumpyDocstring
        )
...
"""
        ...
        for i, error in enumerate(errors):
            ...
            match_re = re.compile(error)
            self.assertTrue(
                bool(match_re.match(warn[0])), f"{error} \n do not match \n {warn[0]}"
            )
            self.assertEqual(
                i + 6, warn[1], msg=f"msg={warn[0]}, docstring='{str(numpy_docstring)}'"
            )
...
:returns: * `bool` - True if successful, False otherwise.
    ...
    def test_fields_blank_lines_sphinx_upstream(self):
        ...
        self.assertAlmostEqualSphinxDocstring(
            expected_wrong, docstring, type_=SphinxNumpyDocstring
        )
