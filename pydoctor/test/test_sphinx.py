...
def test_generateLine_package(inv_writer_nolog: sphinx.SphinxInventoryWriter) -> None:
    ...
    result = inv_writer_nolog._generateLine(model.Package(IGNORE_SYSTEM, 'package1'))
...
def test_generateLine_module(inv_writer_nolog: sphinx.SphinxInventoryWriter) -> None:
    ...
    result = inv_writer_nolog._generateLine(model.Module(IGNORE_SYSTEM, 'module1'))
...
def test_generateLine_class(inv_writer_nolog: sphinx.SphinxInventoryWriter) -> None:
    ...
    result = inv_writer_nolog._generateLine(model.Class(IGNORE_SYSTEM, 'class1'))
...
def test_generateLine_unknown() -> None:
    ...
    result = inv_writer._generateLine(UnknownType(IGNORE_SYSTEM, 'unknown1'))
...
not-valid-zlib-content"""
    ...
    assert [
        (
            'sphinx',
            'Failed to uncompress inventory from http://tm.tld',
            -1,
        )
...
""" + zlib.compress(
    ...
    assert [
        (
            'sphinx',
            'Failed to decode inventory from http://tm.tld',
            -1,
        )
    ] == inv_reader._logger.messages
...
def test_parseInventory_spaces() -> None:
    ...
    assert sphinx._parseInventoryLine(
        ...
        'std:term',
        -1,
        'glossary.html#term-key-function',
        '-',
    )
