...
def test_mro_cycle(capsys: CapSys) -> None:
    ...
    assert (
        ...
        == '''cycle:1: Cycle found while computing inheritance hierarchy: cycle.A -> cycle.D -> cycle.C -> cycle.A
...
def test_overriden_in() -> None:
    ...
    assert (
        stanutils.flatten_text(
            pages.get_override_info(
                simple.contents['A'],  # type:ignore
                'a',
            )
        )
        == 'overridden in normal.C'
    )
    assert (
        stanutils.flatten_text(
            pages.get_override_info(
                simple.contents['B'],  # type:ignore
                'b',
            )
        )
        == 'overridden in normal.C'
    ...
    assert (
        stanutils.flatten_text(
            pages.get_override_info(
                dimond.contents['_MyBase'],  # type:ignore
                'z',
            )
        )
        == 'overridden in diamond.A, diamond.C'
    )
    ...
    ) == ('overrides diamond._MyBase.z' 'overridden in diamond.C')
    assert (
        stanutils.flatten_text(
            pages.get_override_info(
                dimond.contents['C'],  # type:ignore
                'z',
            )
        )
        == 'overrides diamond.A.z'
    )
