...
def test_get_assign_docstring_node() -> None:
    ...
    astutils.Parentage().visit(tree)
    assert (
        astutils.get_str_value(astutils.get_assign_docstring_node(tree.body[0]))
        == "inline docs"
    )  # type:ignore
...
def test_get_assign_docstring_node_not_in_body() -> None:
    ...
    astutils.Parentage().visit(tree)
    assert (
        astutils.get_str_value(
            astutils.get_assign_docstring_node(tree.body[0].orelse[0])
        )
        == "inline docs"
    )  # type:ignore
    ...
    assert (
        astutils.get_str_value(
            astutils.get_assign_docstring_node(tree.body[0].handlers[0].body[0])
        )
        == "inline docs"
    )  # type:ignore
    assert (
    ...
    assert (
        astutils.get_str_value(
            astutils.get_assign_docstring_node(tree.body[0].finalbody[0])
        )
        == "inline docs"
    )  # type:ignore
