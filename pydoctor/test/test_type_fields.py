...
def test_to_node_markup() -> None:
    ...
    cases = [
        ...
        ('I{here}', '*here*'),
        (
            'L{complicated string} or L{strIO <twisted.python.compat.NativeStringIO>}',
...
def test_processtypes(capsys: CapSys) -> None:
    ...
    for strings, excepted_html in cases:
        ...
        assert (
            flatten(
                parse_docstring(rst_string, 'restructuredtext', processtypes=True)
                .fields[-1]
                .body()
                .to_stan(NotFoundLinker())
            )
...
def test_processtypes_corner_cases(capsys: CapSys) -> None:
    ...
    assert (
        ...
        == """<a class="rst-external rst-reference" href="https://github.com" target="_top">hello</a>[<code>str]</code>"""
    )
    assert process('**hello**[str]') == "<strong>hello</strong>[<code>str]</code>"
    assert (
        process('["hello" or str, default: 2]')
        == """[<span class="literal">"hello"</span> or <code>str</code>, <em>default</em>: <span class="literal">2</span>]"""
    )
