...
def test_func_only_single_param_doc() -> None:
    ...
    expected_html = [
        ...
        '</td>',
        '<td class="fieldArgDesc">Actual documentation.</td>',
        '</tr>',
        '<tr>',
        '<td class="fieldArgContainer">',
...
def test_func_raise_linked() -> None:
    ...
    assert (
        '<a href="test.SpanishInquisition.html" class="internal-link" title="test.SpanishInquisition">SpanishInquisition</a>'
        in html
    )
...
def test_func_starargs_more(capsys: CapSys) -> None:
    ...
    mod_rst_with_asterixes = fromText(
        r'''
    def f(args, kwargs, *a, **kwa) -> None:
        r"""
        Do something with var-positional and var-keyword arguments.
...
def test_func_starargs_wrongly_documented(capsys: CapSys) -> None:
    ...
    assert (
        'Documented parameter "stuff" does not exist, variable keywords should be documented with the "Keyword Arguments" section'
        in capsys.readouterr().out
    )

    docstring2html(rst_wrong.contents['f'])
    assert (
        'Documented parameter "stuff" does not exist, variable keywords should be documented with the "keyword" field'
        in capsys.readouterr().out
    )
...
def test_EpydocLinker_switch_context_is_reentrant(
    ...
    warnings = [
        'test:2: Cannot find link target for "thing.notfound" (you can link to external docs with --intersphinx)'
    ]
...
def test_module_docformat_inheritence(capsys: CapSys) -> None:
    ...
    captured = capsys.readouterr().out
    assert not captured
    assert ''.join(docstring2html(top.contents['f']).splitlines()) == ''.join(
        docstring2html(mod.contents['f']).splitlines()
    )
...
def test_constant_values_rst(capsys: CapSys) -> None:
    ...
    expected = (
        '<table class="valueTable"><tr class="fieldStart">'
...
def test_warns_field(capsys: CapSys) -> None:
    ...
    assert (
        '<div><table class="fieldTable"><tr class="fieldStart">'
        '<td class="fieldName" colspan="2">Warns</td></tr><tr>'
        '<td colspan="2">If there is an issue.</td></tr></table></div>'
    ) == html
    captured = capsys.readouterr().out
    assert captured == ''
    ...
    assert (
        ...
        '<td class="fieldArgDesc">If there is an issue.</td></tr></table></div>'
    ) == html
    captured = capsys.readouterr().out
    assert captured == ''
...
def test_insert_break_points_dotted_name() -> None:
    assert (
        insert_break_points('mod.__some_very_long_name__')
        == 'mod<wbr></wbr>.__some<wbr></wbr>_very<wbr></wbr>_long<wbr></wbr>_name__'
    )
    assert (
        insert_break_points('_mod.__SOME_VERY_LONG_NAME__')
        == '_mod<wbr></wbr>.__SOME<wbr></wbr>_VERY<wbr></wbr>_LONG<wbr></wbr>_NAME__'
    )
    assert (
        insert_break_points('pack.mod.__someVeryLongName__')
        == 'pack<wbr></wbr>.mod<wbr></wbr>.__some<wbr></wbr>Very<wbr></wbr>Long<wbr></wbr>Name__'
    )
    assert (
        insert_break_points('pack._mod_.__einÜberlangerName__')
        == 'pack<wbr></wbr>._mod_<wbr></wbr>.__ein<wbr></wbr>Überlanger<wbr></wbr>Name__'
    )


def test_stem_identifier() -> None:
    assert (
        list(stem_identifier('__some_very_long_name__'))
        == list(stem_identifier('__some_very_very_long_name__'))
        == [
            'some',
            'very',
            'long',
...
~~~~~</p>
    ...
    captured = capsys.readouterr().out
    assert captured == (
        'invalid:4: bad docstring: Wrong underline character for heading.\n'
