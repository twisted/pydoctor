...
from pydoctor import stanutils
...
def test_overload(systemcls: Type[model.System]) -> None:
    ...
    assert stanutils.flatten_text(stanutils.html2stan(str(func.signature).replace(' ', ''))) == "(s:Union[str,bytes])->Union[str,bytes]"
    assert [astbuilder.node2dottedname(d) for d in (func.decorators or ())] == []
    overloads = func.overloads
    assert len(overloads)==2
    assert stanutils.flatten_text(stanutils.html2stan(str(overloads[0].signature).replace(' ', ''))) == "(s:str)->str"
    assert stanutils.flatten_text(stanutils.html2stan(str(overloads[1].signature).replace(' ', ''))) == "(s:bytes)->bytes"
