...
def fromText(
    text, modname="<test>", system=None, buildercls=None, systemcls=model.System
):
    if system is None:
        _system = systemcls()
    else:
...
def test_subclasses():
    ...
    assert system.allobjects["<test>.A"].subclasses == [system.allobjects["<test>.B"]]
...
def test_classdecorator():
    ...
    C = mod.contents["C"]
    assert C.decorators == [(("cd", "mod.cd", mod.contents["cd"]), None)], C.decorators
...
def test_classdecorator_with_args():
    ...
    A = mod.contents["A"]
    C = mod.contents["C"]
    assert C.decorators == [(("cd", "test.cd", cd), [("A", "test.A", A)])], C.decorators
